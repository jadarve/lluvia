...
function builder.newDescriptor() 
    ...
    desc:addPort(ll.PortDescriptor.new(3, 'in_flow', ll.PortDirection.In, ll.PortType.SampledImageView))
    ...
    -- the loop between FlowPredict and FlowUpdateDelta can be broken.
    desc:addPort(ll.PortDescriptor.new(5, 'out_flow', ll.PortDirection.Out, ll.PortType.ImageView))
    desc:addPort(ll.PortDescriptor.new(6, 'out_delta_flow', ll.PortDirection.Out, ll.PortType.ImageView))
...
function builder.onNodeInit(node)
    ...
    -- ll.logd('FlowUpdateDelta', 'in_gray ', string.format('[%d, %d, %d]', in_gray.width, in_gray.height, in_gray.channelCount)
