...
cdef class Memory:
    ...
    def createBuffer(self, uint64_t size,
                     usageFlags=[ll_buffer.BufferUsageFlagBits.StorageBuffer,
        ...
        cdef uint32_t flattenFlags = impl.flattenFlagBits(usageFlags, ll_buffer.BufferUsageFlagBits)
        cdef _BufferUsageFlags _usageFlags = <_BufferUsageFlags> flattenFlags

        return _buildBuffer(self.__memory.get().createBuffer(size, _usageFlags), self.session, self)
    ...
    def createBufferFromHost(self, np.ndarray arr,
        ...
        cdef Buffer buf = self.createBuffer(arr.nbytes, usageFlags)
        buf.fromHost(arr)
        return buf
